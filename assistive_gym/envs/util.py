...
class Util:
    ...
    def enable_gpu(self, visible=False):
    ...
    def ik_random_restarts(
        ...
        for r in range(max_ik_random_restarts):
            ...
            world_creation.setup_robot_joints(
                body,
                robot_arm_joint_indices,
                robot_lower_limits,
                robot_upper_limits,
                randomize_joint_positions=False,
                default_positions=np.array(target_joint_positions),
                tool=None,
            )
            if step_sim:
                for _ in range(5):
                ...
                if (
                    len(p.getContactPoints(bodyA=body, bodyB=body, physicsClientId=self.id)) > 0
                    and orient_orig is not None
                ):
                    ...
                    target_orient = p.getQuaternionFromEuler(
                        p.getEulerFromQuaternion(orient_orig, physicsClientId=self.id)
                        + np.deg2rad(self.np_random.uniform(-45, 45, size=3)),
            ...
            gripper_pos, gripper_orient = p.getLinkState(
                body, target_joint, computeForwardKinematics=True, physicsClientId=self.id
            )[:2]
            if np.linalg.norm(target_pos - np.array(gripper_pos)) < random_restart_threshold and (
                target_orient is None
                or np.linalg.norm(target_orient - np.array(gripper_orient)) < random_restart_threshold
                or np.isclose(
        ...
        world_creation.setup_robot_joints(
            ...
            robot_upper_limits,
            randomize_joint_positions=False,
            default_positions=np.array(best_ik_joints),
            tool=None,
        )
    ...
    def ik_jlwki(
        ...
        if np.linalg.norm(target_pos - np.array(gripper_pos)) < success_threshold and (
            target_orient is None
            or np.linalg.norm(target_orient - np.array(gripper_orient)) < success_threshold
            or np.isclose(np.linalg.norm(target_orient - np.array(gripper_orient)), 2, atol=success_threshold)
        ):
            return True, np.array(target_joint_positions)
        return False, np.array(target_joint_positions)
    ...
    def capsule_points(self, p1, p2, radius, distance_between_points=0.05):
        ...
        for i, section_pos in enumerate(section_positions):
            ...
            for j in range(int(circumference / distance_between_points)):
                ...
                point_on_circle = (
                    p1 + section_pos + radius * np.cos(theta) * ortho_vector + radius * np.sin(theta) * normal_vector
                )
    ...
    def sleeve_on_arm_reward(
        self, triangle1_points, triangle2_points, human, hand_radius, elbow_radius, shoulder_radius
    ):
        shoulder_pos, shoulder_orient = p.getLinkState(
            human, 15, computeForwardKinematics=True, physicsClientId=self.id
        )[:2]
        ...
        shoulder_end_pos = (
            shoulder_pos + (shoulder_pos - elbow_pos) / np.linalg.norm(shoulder_pos - elbow_pos) * shoulder_radius
        )
        ...
        forearm_intersects_triangle1 = self.line_intersects_triangle(
            triangle1_points[0], triangle1_points[1], triangle1_points[2], hand_end_pos, elbow_end_pos
        )
        forearm_intersects_triangle2 = self.line_intersects_triangle(
            triangle2_points[0], triangle2_points[1], triangle2_points[2], hand_end_pos, elbow_end_pos
        )
        upperarm_intersects_triangle1 = self.line_intersects_triangle(
            triangle1_points[0], triangle1_points[1], triangle1_points[2], elbow_end_pos, shoulder_end_pos
        )
        upperarm_intersects_triangle2 = self.line_intersects_triangle(
            triangle2_points[0], triangle2_points[1], triangle2_points[2], elbow_end_pos, shoulder_end_pos
        )
