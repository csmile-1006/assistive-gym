...
class Robot(Agent):
    def __init__(
        ...
        self.right_gripper_collision_indices = (
            right_gripper_collision_indices  # Used to disable collision between gripper and tools
        )
        self.left_gripper_collision_indices = (
            left_gripper_collision_indices  # Used to disable collision between gripper and tools
        )
    ...
    def ik_random_restarts(
        ...
        for r in range(max_ik_random_restarts):
            ...
            self.set_joint_angles(
                self.right_arm_joint_indices if right else self.left_arm_joint_indices, target_joint_angles
            )
    ...
    def position_robot_toc(
        ...
        while iteration < attempts or best_position is None:
            ...
            self.set_base_pos_orient(
                np.array([-0.85, -0.4, 0]) + self.toc_base_pos_offset[task] + random_pos, random_orientation
            )
        ...
        p.resetBasePositionAndOrientation(
            self.body,
            np.array([-0.85, -0.4, 0]) + np.array(self.toc_base_pos_offset[task]) + best_position,
            best_orientation,
            physicsClientId=self.id,
        )
        for i, arm in enumerate(arms):
            self.set_joint_angles(
                self.right_arm_joint_indices if arm == "right" else self.left_arm_joint_indices,
