...
class DrinkingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        info = {
            ...
            "task_success": int(self.task_success >= self.total_water_count * self.config("task_success_threshold")),
    ...
    def get_water_rewards(self):
        ...
        top_center_pos, _ = p.multiplyTransforms(
            cup_pos, cup_orient, self.cup_top_center_offset, [0, 0, 0, 1], physicsClientId=self.id
        )
        bottom_center_pos, _ = p.multiplyTransforms(
            cup_pos, cup_orient, self.cup_bottom_center_offset, [0, 0, 0, 1], physicsClientId=self.id
        )
        ...
        if self.cup_top_center is not None:
            p.resetBasePositionAndOrientation(
                self.cup_top_center, top_center_pos, [0, 0, 0, 1], physicsClientId=self.id
            )
            p.resetBasePositionAndOrientation(
                self.cup_bottom_center, bottom_center_pos, [0, 0, 0, 1], physicsClientId=self.id
            )
        ...
        for w in self.waters:
            ...
            if not self.util.points_in_cylinder(top_center_pos, bottom_center_pos, 0.05, np.array(water_pos)):
                ...
                if distance_to_mouth < 0.03:  # hard
                    ...
                    self.task_success += 1
                    p.resetBasePositionAndOrientation(
                        w, self.np_random.uniform(1000, 2000, size=3), [0, 0, 0, 1], physicsClientId=self.id
                    )
    ...
    def reset(self, randomness_values=None):
        ...
        joints_positions += [
            (21, np.deg2rad(self._randomness_values[5])),
            (22, np.deg2rad(self._randomness_values[6])),
            (23, np.deg2rad(self._randomness_values[7])),
        ]
        ...
        sphere_visual = p.createVisualShape(
            shapeType=p.GEOM_SPHERE, radius=0.01, rgbaColor=[0, 1, 0, 1], physicsClientId=self.id
        )
        ...
        target_pos = np.array([-0.2, -0.5, 1]) + self._randomness_values[8]
        if self.robot_type == "pr2":
            ...
            self.cup = self.world_creation.init_tool(
                self.robot,
                mesh_scale=[0.045] * 3,
                pos_offset=[-0.01, 0, -0.05],
                orient_offset=p.getQuaternionFromEuler([np.pi / 2.0, 0, 0], physicsClientId=self.id),
                left=False,
                maximal=False,
                alpha=0.75,
            )
            ...
            self.cup = self.world_creation.init_tool(
                self.robot,
                mesh_scale=[0.045] * 3,
                pos_offset=[0.05, -0.005, 0],
                orient_offset=p.getQuaternionFromEuler([0, 0, np.pi / 2.0], physicsClientId=self.id),
                left=False,
                maximal=False,
                alpha=0.75,
            )
            ...
            for j in range(4):
                for k in range(4):
                    batch_positions.append(
                        np.array(
                            [i * 2 * water_radius - 0.02, j * 2 * water_radius - 0.02, k * 2 * water_radius + 0.075]
                        )
                        + cup_pos
                    )
        last_water_id = p.createMultiBody(
            baseMass=water_mass,
        ...
        for w in self.waters:
            p.changeVisualShape(w, -1, rgbaColor=[1, 0, 1, 1], physicsClientId=self.id)
    ...
    def display_cup_points(self):
        sphere_collision = -1
        sphere_visual = p.createVisualShape(
            shapeType=p.GEOM_SPHERE, radius=0.01, rgbaColor=[0, 1, 1, 1], physicsClientId=self.id
        )
        cup_pos, cup_orient = p.getBasePositionAndOrientation(self.cup, physicsClientId=self.id)
        cup_pos, cup_orient = p.multiplyTransforms(
            cup_pos,
        ...
        self.cup_top_center = p.createMultiBody(
            baseMass=0.0,
            baseCollisionShapeIndex=sphere_collision,
            baseVisualShapeIndex=sphere_visual,
            basePosition=top_center_pos,
            useMaximalCoordinates=False,
            physicsClientId=self.id,
        )
        self.cup_bottom_center = p.createMultiBody(
            baseMass=0.0,
            baseCollisionShapeIndex=sphere_collision,
            baseVisualShapeIndex=sphere_visual,
            basePosition=bottom_center_pos,
            useMaximalCoordinates=False,
            physicsClientId=self.id,
        )
        ...
        cylinder_visual = p.createVisualShape(
            shapeType=p.GEOM_CYLINDER, radius=0.05, length=0.14, rgbaColor=[0, 1, 1, 0.25], physicsClientId=self.id
        )
        self.cup_cylinder = p.createMultiBody(
            baseMass=0.0,
            baseCollisionShapeIndex=cylinder_collision,
            baseVisualShapeIndex=cylinder_visual,
            basePosition=cup_pos,
            baseOrientation=cup_orient,
            useMaximalCoordinates=False,
            physicsClientId=self.id,
        )
    ...
    def compute_reward(self, action):
        ...
        # Return Home Penalty (r_return_home):
