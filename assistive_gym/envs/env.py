...
class AssistiveEnv(gym.Env):
    def __init__(
        ...
        self.action_space = spaces.Box(
            low=np.array([-1.0] * (self.action_robot_len + self.action_human_len)),
            high=np.array([1.0] * (self.action_robot_len + self.action_human_len)),
            dtype=np.float32,
        )
        self.observation_space = spaces.Box(
            low=np.array([-1.0] * (self.obs_robot_len + self.obs_human_len)),
        ...
        # self.width = 1920 // 4
        # self.height = 1080 // 4
        self.width, self.height = 1920 // 6, 1080 // 6
    ...
    def take_step(self, action, robot_arm="left", gains=0.05, forces=1, human_gains=0.1, human_forces=1, step_sim=True):
        ...
        indices = (
            ...
            if robot_arm == "left"
            else self.robot_right_arm_joint_indices if robot_arm == "right" else self.robot_both_arm_joint_indices
        )
        ...
        if self.human_control or (
            ...
            if len(action_human) != human_len:
                print(
                    "Received human actions of length %d does not match expected action length of %d"
    ...
    def enforce_realistic_human_joint_limits(self):
        ...
        if 3 in self.human_controllable_joint_indices:
            # Right human arm
            tz, tx, ty, qe = (
                j[0] for j in p.getJointStates(self.human, jointIndices=[3, 4, 5, 6], physicsClientId=self.id)
            )
            ...
            if result == 1:
                ...
                for i, j in enumerate([3, 4, 5, 6]):
                    p.resetJointState(
                        self.human,
                        jointIndex=j,
                        targetValue=self.right_arm_previous_valid_pose[i],
                        targetVelocity=0,
                        physicsClientId=self.id,
                    )
        if 13 in self.human_controllable_joint_indices:
            # Left human arm
            tz, tx, ty, qe = (
                j[0] for j in p.getJointStates(self.human, jointIndices=[13, 14, 15, 16], physicsClientId=self.id)
            )
            # Transform joint angles to match those from the Matlab data
            tz2 = (tz + 2 * np.pi) % (2 * np.pi)
            tx2 = (tx + 2 * np.pi) % (2 * np.pi)
            ...
            if result == 1:
                ...
                for i, j in enumerate([13, 14, 15, 16]):
                    p.resetJointState(
                        self.human,
                        jointIndex=j,
                        targetValue=self.left_arm_previous_valid_pose[i],
                        targetVelocity=0,
                        physicsClientId=self.id,
                    )
        ...
        for i, j in enumerate(self.human_controllable_joint_indices):
            if joint_positions[i] < self.human_joint_lower_limits[i]:
                p.resetJointState(
                    self.human,
                    jointIndex=j,
                    targetValue=self.human_joint_lower_limits[i],
                    targetVelocity=0,
                    physicsClientId=self.id,
                )
                ...
                p.resetJointState(
                    self.human,
                    jointIndex=j,
                    targetValue=self.human_joint_upper_limits[i],
                    targetVelocity=0,
                    physicsClientId=self.id,
                )
    ...
    def human_preferences(
        ...
        if self.task in ["arm_manipulation"]:
                ...
                p.getClosestPoints(
                    ...
                    linkIndexA=(
                        ...
                        if self.robot_type == "pr2"
                            ...
                            else 54 if self.robot_type == "baxter" else 9 if self.robot_type == "jaco" else 7
                        )
                    ),
                    distance=0.01,
                    physicsClientId=self.id,
                ...
                p.getClosestPoints(
                    ...
                    linkIndexA=(
                        ...
                        if self.robot_type == "pr2"
                            ...
                            24
                            if self.robot_type == "sawyer"
                            else 31 if self.robot_type == "baxter" else 9 if self.robot_type == "jaco" else 7
                        )
                    ),
                    distance=0.01,
                    physicsClientId=self.id,
            ...
            tool_left_pressure = (
                ...
                else (arm_manipulation_tool_forces_on_human[0] / tool_left_contact_points)
            )
            tool_right_pressure = (
                0
                if tool_right_contact_points <= 0
                else (arm_manipulation_tool_forces_on_human[1] / tool_right_contact_points)
            )
    ...
    def reset_robot_joints(self):
        ...
        # Position end effectors with dual arm robots
        if self.robot_type == "pr2":
            ...
            for i, j in enumerate(self.robot_left_arm_joint_indices):
                p.resetJointState(
                    self.robot,
                    jointIndex=j,
                    targetValue=[0.75, 1, 0.5, 0.5, 1, -0.5, 0][i],
                    targetVelocity=0,
                    physicsClientId=self.id,
                )
            for i, j in enumerate(self.robot_right_arm_joint_indices):
    ...
    def joint_limited_weighting(self, q, lower_limits, upper_limits):
        ...
        lam = 0.05
        weights = []
        for qi, l, u in zip(q, lower_limits, upper_limits):  # noqa
            qr = 0.5 * (u - l)
    ...
    def position_robot_toc(
        ...
        while iteration < attempts or best_position is None:
            ...
            p.resetBasePositionAndOrientation(
                robot, np.array([-0.85, -0.4, 0]) + pos_offset + random_pos, random_orientation, physicsClientId=self.id
            )
                    ...
                    for k in range(ik_random_restarts):
                        ...
                        if human_joint_positions is not None:
                            for h, pos in zip(human_joint_indices, human_joint_positions):
                                p.resetJointState(
                                    self.human, jointIndex=h, targetValue=pos, targetVelocity=0, physicsClientId=self.id
                                )
                        # Reset all robot joints
                        self.reset_robot_joints()
                        # Find IK solution
                        success, joint_positions_q_star = self.util.ik_jlwki(
                            robot,
                            joint,
                            target_pos,
                            orient,
                        ...
                        joint_limit_weight = self.joint_limited_weighting(
                            joint_positions_q_star, lower_limits[i], upper_limits[i]
                        )
            ...
            if num_goals_reached > 0:
                if (
                    ...
                    or (num_goals_reached == best_num_goals_reached and manipulability > best_manipulability)
                ):
        ...
        p.resetBasePositionAndOrientation(
            robot, np.array([-0.85, -0.4, 0]) + pos_offset + best_position, best_orientation, physicsClientId=self.id
        )
        for i, joint in enumerate(joints):
    ...
    def get_camera_image_depth(self, view, light_pos=[0, -3, 1], shadow=False, ambient=0.8, diffuse=0.3, specular=0.1):
        assert (
            self.view_matrices is not None and self.projection_matrices is not None
        ), "You must call env.setup_camera() or env.setup_camera_rpy() before getting a camera image"
        if self.view_matrices[view] is None or self.projection_matrices[view] is None:
            view_matrix, projection_matrix = self.setup_egocentric_camera()
        else:
