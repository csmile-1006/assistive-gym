...
class AssistiveEnv(gym.Env):
    def __init__(
        ...
        # logdir for recording videos
        self.record_video = record_video
        self.video_writer = {key: None for key in self.view_matrices.keys()}

        if self.record_video:
            self.log_dir = os.path.join(
                Path(os.path.realpath(__file__)).parent.parent.parent,
                "logs",
                f"{self.task}_{self.robot.name}",
                "videos",
    ...
    def reset(self):
        ...
        self.task_success = 0
        if self.record_video:
            self.setup_record_video()

    def build_assistive_env(
        self, furniture_type=None, fixed_human_base=True, human_impairment="random", gender="random"
    ):
    ...
    def update_action_space(self):
        ...
        self.action_space.__init__(
            low=-np.ones(action_len, dtype=np.float32), high=np.ones(action_len, dtype=np.float32), dtype=np.float32
        )
    ...
    def human_preferences(
        ...
        if self.task == "arm_manipulation":
            ...
            tool_right_pressure = (
                ...
                else (arm_manipulation_tool_forces_on_human[0] / tool_right_contact_points)
            )
            tool_left_pressure = (
                0
                if tool_left_contact_points <= 0
                else (arm_manipulation_tool_forces_on_human[1] / tool_left_contact_points)
            )
    ...
    def get_camera_image_depth(
        self, view=None, light_pos=[0, -3, 1], shadow=False, ambient=0.8, diffuse=0.3, specular=0.1
    ):
        if view is None:
            view = list(self.view_matrices.keys())[0]
        assert (
            self.view_matrices[view] is not None and self.projection_matrices[view] is not None
        ...
        img = np.reshape(img, (h, w, 4)).astype(np.uint8)
        depth = np.reshape(depth, (h, w)).astype(np.float32)
        return img, depth
        ...
        if self.record_video:
            ...
            for view in self.view_matrices:
                if self.video_writer[view] is not None:
                    self.video_writer[view].release()
                self.video_writer[view] = cv2.VideoWriter(
                    os.path.join(self.log_dir, f"{self.task}_{self.robot.name}_{date}_{view}.mp4"),
    ...
    def record_video_frame(self):
        if self.record_video:
            for view in self.view_matrices:
                ...
                self.video_writer[view].write(frame)
    ...
    def create_sphere(
        ...
        sphere_collision = (
            p.createCollisionShape(shapeType=p.GEOM_SPHERE, radius=radius, physicsClientId=self.id) if collision else -1
        )
        sphere_visual = (
            p.createVisualShape(shapeType=p.GEOM_SPHERE, radius=radius, rgbaColor=rgba, physicsClientId=self.id)
            if visual
            else -1
        )
    ...
    def create_spheres(
        self, radius=0.01, mass=0.0, batch_positions=[[0, 0, 0]], visual=True, collision=True, rgba=[0, 1, 1, 1]
    ):
        sphere_collision = (
            p.createCollisionShape(shapeType=p.GEOM_SPHERE, radius=radius, physicsClientId=self.id) if collision else -1
        )
    ...
    def create_agent_from_obj(
        ...
        visual_shape = p.createVisualShape(
            shapeType=p.GEOM_MESH, fileName=visual_filename, meshScale=scale, rgbaColor=rgba, physicsClientId=self.id
        )
        collision_shape = p.createCollisionShape(
            shapeType=p.GEOM_MESH, fileName=collision_filename, meshScale=scale, physicsClientId=self.id
        )
        body = p.createMultiBody(
            baseMass=mass,
