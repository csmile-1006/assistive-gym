...
class BedBathingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        end_effector_velocity = np.linalg.norm(
            p.getLinkState(
                self.tool, 1, computeForwardKinematics=True, computeLinkVelocity=True, physicsClientId=self.id
            )[6]
        )
        ...
        reward_distance = -min(
            [c[8] for c in p.getClosestPoints(self.tool, self.human, distance=4.0, physicsClientId=self.id)]
        )
        ...
        for c in p.getContactPoints(bodyA=self.tool, bodyB=self.human, physicsClientId=self.id):
            ...
            if linkA in [1]:
                ...
                for i, (target_pos_world, target) in enumerate(
                    ...
                    if np.linalg.norm(contact_position - target_pos_world) < 0.025:
                        ...
                        p.resetBasePositionAndOrientation(
                            target, [1000, 1000, 1000], [0, 0, 0, 1], physicsClientId=self.id
                        )
                        indices_to_delete.append(i)
                self.targets_pos_on_upperarm = [
                    t for i, t in enumerate(self.targets_pos_on_upperarm) if i not in indices_to_delete
                ]
                ...
                self.targets_pos_upperarm_world = [
                    t for i, t in enumerate(self.targets_pos_upperarm_world) if i not in indices_to_delete
                ]
                ...
                self.targets_pos_on_forearm = [
                    t for i, t in enumerate(self.targets_pos_on_forearm) if i not in indices_to_delete
                ]
                ...
                self.targets_pos_forearm_world = [
                    t for i, t in enumerate(self.targets_pos_forearm_world) if i not in indices_to_delete
                ]
    ...
    def _get_obs(self, forces, forces_human):
        ...
        robot_joint_states = p.getJointStates(
            self.robot, jointIndices=self.robot_left_arm_joint_indices, physicsClientId=self.id
        )
    ...
    def reset(self):
        ...
        self.world_creation.setup_human_joints(
            self.human, joints_positions, controllable_joints, use_static_joints=False, human_reactive_force=None
        )
        p.resetBasePositionAndOrientation(
            self.human,
            [-0.15, 0.2, 0.95],
        ...
        for j in range(p.getNumJoints(self.human, physicsClientId=self.id)):
            if p.getJointInfo(self.human, j, physicsClientId=self.id)[2] != p.JOINT_FIXED:
                p.resetJointState(
                    self.human,
                    jointIndex=j,
                    targetValue=self.np_random.uniform(-0.1, 0.1),
                    targetVelocity=0,
                    physicsClientId=self.id,
                )
        ...
        if self.robot_type == "pr2":
            ...
            self.tool = self.world_creation.init_tool(
                ...
                mesh_scale=[1] * 3,
                pos_offset=[0, 0, 0],
                orient_offset=p.getQuaternionFromEuler([0, 0, 0], physicsClientId=self.id),
                maximal=False,
            )
            ...
            self.tool = self.world_creation.init_tool(
                ...
                orient_offset=p.getQuaternionFromEuler([0, -np.pi / 2.0, 0], physicsClientId=self.id),
                maximal=False,
            )
            ...
            self.tool = self.world_creation.init_tool(
                ...
                orient_offset=p.getQuaternionFromEuler([np.pi / 2.0, 0, np.pi / 2.0], physicsClientId=self.id),
                maximal=False,
            )
    ...
    def update_targets(self):
        upperarm_pos, upperarm_orient = p.getLinkState(
            self.human, self.upperarm, computeForwardKinematics=True, physicsClientId=self.id
        )[:2]
        self.targets_pos_upperarm_world = []
        for target_pos_on_arm, target in zip(self.targets_pos_on_upperarm, self.targets_upperarm):
            target_pos = np.array(
                p.multiplyTransforms(
                    upperarm_pos, upperarm_orient, target_pos_on_arm, [0, 0, 0, 1], physicsClientId=self.id
                )[0]
            )
        ...
        forearm_pos, forearm_orient = p.getLinkState(
            self.human, self.forearm, computeForwardKinematics=True, physicsClientId=self.id
        )[:2]
        self.targets_pos_forearm_world = []
        for target_pos_on_arm, target in zip(self.targets_pos_on_forearm, self.targets_forearm):
            target_pos = np.array(
                p.multiplyTransforms(
                    forearm_pos, forearm_orient, target_pos_on_arm, [0, 0, 0, 1], physicsClientId=self.id
                )[0]
            )
