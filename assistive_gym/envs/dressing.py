...
class DressingEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super().__init__(
            ...
            task="dressing",
            obs_robot_len=(
                17 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)
            ),
    ...
    def _get_obs(self, agent=None):
        ...
        end_effector_pos_real, end_effector_orient_real = self.robot.convert_to_realworld(
        ...
        if self.human.controllable:
            ...
            end_effector_pos_human, end_effector_orient_human = self.human.convert_to_realworld(
                end_effector_pos, end_effector_orient
            )
    ...
    def reset(self):
        ...
        self.robot.set_gripper_open_position(
            self.robot.left_gripper_indices, self.robot.gripper_pos[self.task], set_instantly=True
        )
        ...
        # For visualizing the sleeve on the arm
        # Load the object as a visual shape
        visualShapeId = p.createVisualShape(
            p.GEOM_MESH, fileName=os.path.join(self.directory, "clothing", "hospitalgown_reduced.obj")
        )
        # Load as a visual object at a fixed position
        self.cloth_visual = p.createMultiBody(baseMass=0, baseVisualShapeIndex=visualShapeId)
