...
class DressingEnv(AssistiveEnv):
    ...
    def step(self, action):
        ...
        for _ in range(self.frame_skip):
            ...
            state = p.getLinkState(
                ...
                (
                    ...
                    else 19 if self.robot_type == "sawyer" else 48 if self.robot_type == "baxter" else 8
                ),
        ...
        (
            ...
            triangle2_points,
            self.human,
            self.world_creation.human_creation.hand_radius,
            self.world_creation.human_creation.elbow_radius,
        ...
        forces = (
            np.concatenate(
                [np.expand_dims(fx, axis=-1), np.expand_dims(fy, axis=-1), np.expand_dims(fz, axis=-1)], axis=-1
            )
            * 10
        )
        contact_positions = np.concatenate(
            [np.expand_dims(cx, axis=-1), np.expand_dims(cy, axis=-1), np.expand_dims(cz, axis=-1)], axis=-1
        )
        forces_temp = []
    ...
    def _get_obs(self, forces, forces_human):
        ...
        shoulder_pos, shoulder_orient = p.getLinkState(
            self.human, 15, computeForwardKinematics=True, physicsClientId=self.id
        )[:2]
        elbow_pos, elbow_orient = p.getLinkState(
            self.human, 17, computeForwardKinematics=True, physicsClientId=self.id
        )[:2]
        wrist_pos, wrist_orient = p.getLinkState(
            self.human, 19, computeForwardKinematics=True, physicsClientId=self.id
        )[:2]
    ...
    def reset(self):
        ...
        gripper_visual = p.createVisualShape(
            p.GEOM_SPHERE, radius=0.01, rgbaColor=[0, 0, 0, 0], physicsClientId=self.id
        )
        ...
        self.cloth = p.loadCloth(
            os.path.join(self.world_creation.directory, "clothing", "hospitalgown_reduced.obj"),
            scale=1.4,
            mass=0.23,
            position=np.array([0.02, -0.38, 0.83]) + self.cloth_offset / 1.4,
            orientation=p.getQuaternionFromEuler([0, 0, np.pi], physicsClientId=self.id),
            bodyAnchorId=self.cloth_attachment,
            anchors=[2087, 3879, 3681, 3682, 2086, 2041, 987, 2042, 2088, 1647, 2332],
            collisionMargin=0.04,
            # rgbaColor=np.array([139.0 / 256.0, 195.0 / 256.0, 74.0 / 256.0, 1]),
            # rgbaColor=np.array([255.0 / 256.0, 69.0 / 256.0, 0.0 / 256.0, 1]),
            # rgbaColor=np.array([255.0 / 256.0, 0.0 / 256.0, 0.0 / 256.0, 1]),
            # rgbaColor=np.array([0.0 / 256.0, 255.0 / 256.0, 255.0 / 256.0, 1]),
            rgbaColor=np.array([255.0 / 256.0, 100.0 / 256.0, 0.0 / 256.0, 1]),
            # rgbaLineColor=np.array([197.0 / 256.0, 225.0 / 256.0, 165.0 / 256.0, 1]),
            # rgbaLineColor=np.array([0.0 / 256.0, 191.0 / 256.0, 255.0 / 256.0, 1]),
            rgbaLineColor=np.array([255.0 / 256.0, 255.0 / 256.0, 255.0 / 256.0, 1]),
            physicsClientId=self.id,
        )
        p.clothParams(
            self.cloth,
        ...
        # Load the object as a visual shape
        visualShapeId = p.createVisualShape(
            p.GEOM_MESH, fileName=os.path.join(self.world_creation.directory, "clothing", "hospitalgown_reduced.obj")
        )
        # Load as a visual object at a fixed position
        self.cloth_visual = p.createMultiBody(baseVisualShapeIndex=visualShapeId)
        ...
        for _ in range(200):
            # Force the cloth attachment to stay at the end effector
            p.resetBasePositionAndOrientation(
                self.cloth_attachment, self.start_ee_pos, [0, 0, 0, 1], physicsClientId=self.id
            )
            p.stepSimulation(physicsClientId=self.id)
