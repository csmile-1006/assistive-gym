...
class WorldCreation:
    ...
    def create_new_world(
        ...
        if furniture_type == "wheelchair":
            ...
            if self.robot_type in ["jaco", "kinova_gen3"]:
                furniture = p.loadURDF(
                    os.path.join(
                        self.directory,
                        "wheelchair",
                        "wheelchair_jaco.urdf" if self.task not in ["dressing"] else "wheelchair_jaco_left.urdf",
                    ),
                    physicsClientId=self.id,
                )
            else:
                furniture = p.loadURDF(
                    os.path.join(self.directory, "wheelchair", "wheelchair.urdf"), physicsClientId=self.id
                )
            ...
            bed_collision = p.createCollisionShape(
                shapeType=p.GEOM_MESH,
                fileName=os.path.join(self.directory, "bed", "bed_single_reduced_vhacd.obj"),
                meshScale=mesh_scale,
                flags=p.GEOM_FORCE_CONCAVE_TRIMESH,
                physicsClientId=self.id,
            )
            furniture = p.createMultiBody(
                baseMass=0,
            ...
            furniture = p.loadURDF(
                os.path.join(self.directory, "table", "table.urdf"),
                basePosition=[0, -0.35, 0],
                baseOrientation=p.getQuaternionFromEuler([0, 0, np.pi / 2.0], physicsClientId=self.id),
                physicsClientId=self.id,
            )
        else:
            furniture = None
        ...
        human, human_lower_limits, human_upper_limits = self.init_human(
            static_human_base, self.human_limit_scale, print_joints, gender=gender
        )
        ...
        if self.robot_type == "pr2":
            (
                ...
                robot_lower_limits,
                robot_upper_limits,
                robot_right_arm_joint_indices,
                robot_left_arm_joint_indices,
            ) = self.init_pr2(print_joints)
            ...
            (
                ...
                robot_right_arm_joint_indices,
                robot_left_arm_joint_indices,
            ) = self.init_kinova_gen3(print_joints)
        else:
            (
                robot,
                robot_lower_limits,
    ...
    def init_human(self, static_human_base=False, limit_scale=1.0, print_joints=False, gender="random"):
        human = self.human_creation.create_human(
    ...
    def enforce_joint_limits(self, body):
        ...
        joint_states = p.getJointStates(
            body, jointIndices=list(range(p.getNumJoints(body, physicsClientId=self.id))), physicsClientId=self.id
        )
        ...
        for j in range(p.getNumJoints(body, physicsClientId=self.id)):
            ...
            if joint_pos < lower_limit:
                p.resetJointState(
                    body, jointIndex=j, targetValue=lower_limit, targetVelocity=0, physicsClientId=self.id
                )
            elif joint_pos > upper_limit:
                p.resetJointState(
                    body, jointIndex=j, targetValue=upper_limit, targetVelocity=0, physicsClientId=self.id
                )
        lower_limits = np.array(lower_limits)
        upper_limits = np.array(upper_limits)
        return lower_limits, upper_limits

    def setup_human_joints(
        self,
        human,
        joints_positions,
        controllable_joints,
        ...
        for j in range(p.getNumJoints(human, physicsClientId=self.id)):
            ...
            for j_index, j_angle in joints_positions:
                if j == j_index:
                    p.resetJointState(
                        human, jointIndex=j, targetValue=j_angle, targetVelocity=0, physicsClientId=self.id
                    )
        ...
        for j in range(p.getNumJoints(human, physicsClientId=self.id)):
            p.setJointMotorControl2(
                human, jointIndex=j, controlMode=p.VELOCITY_CONTROL, force=0, physicsClientId=self.id
            )
        ...
        if human_reactive_force is not None:
            ...
            p.setJointMotorControlArray(
                ...
                positionGains=np.array([human_reactive_gain] * len(forces)),
                forces=forces,
                physicsClientId=self.id,
            )
    ...
    def init_pr2(self, print_joints=False):
        if self.task == "arm_manipulation":
            robot = p.loadURDF(
                os.path.join(self.directory, "PR2", "pr2_no_torso_lift_tall_arm_manipulation.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
                physicsClientId=self.id,
            )
            ...
            robot = p.loadURDF(
                os.path.join(self.directory, "PR2", "pr2_no_torso_lift_tall.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
    ...
    def init_sawyer(self, print_joints=False):
        ...
        if self.task == "arm_manipulation":
            robot = p.loadURDF(
                os.path.join(self.directory, "sawyer", "sawyer_arm_manipulation.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
            ...
            robot = p.loadURDF(
                os.path.join(self.directory, "sawyer", "sawyer.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
                flags=p.URDF_USE_SELF_COLLISION,
                physicsClientId=self.id,
    ...
    def init_baxter(self, print_joints=False):
        if self.task == "arm_manipulation":
            robot = p.loadURDF(
                os.path.join(self.directory, "baxter", "baxter_custom_arm_manipulation.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
                physicsClientId=self.id,
            )
            ...
            robot = p.loadURDF(
                os.path.join(self.directory, "baxter", "baxter_custom.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
                physicsClientId=self.id,
            )
    ...
    def init_jaco(self, print_joints=False):
        ...
        if self.task == "arm_manipulation":
            robot = p.loadURDF(
                os.path.join(self.directory, "jaco", "j2s7s300_gym_arm_manipulation.urdf"),
                useFixedBase=True,
                basePosition=[0, 0, 0],
                flags=p.URDF_USE_SELF_COLLISION,
                physicsClientId=self.id,
    ...
    def init_kinova_gen3(self, print_joints=False):
        robot = p.loadURDF(
            os.path.join(self.directory, "kinova_gen3", "GEN3_URDF_V12.urdf"),
            useFixedBase=True,
            basePosition=[0, 0, 0],
            flags=p.URDF_USE_SELF_COLLISION,
            physicsClientId=self.id,
    ...
    def set_gripper_open_position(self, robot, position=0, left=True, set_instantly=False, indices=None):
        ...
        if set_instantly:
            for i, j in enumerate(indices):
                p.resetJointState(
                    robot, jointIndex=j, targetValue=positions[i], targetVelocity=0, physicsClientId=self.id
                )
        p.setJointMotorControlArray(
            robot,
    ...
    def init_tool(
        ...
        if left:
            gripper_pos, gripper_orient = p.getLinkState(
                ...
                (
                    76
                    if self.robot_type == "pr2"
                    else 18 if self.robot_type == "sawyer" else 47 if self.robot_type == "baxter" else 8
                ),
            ...
            gripper_pos, gripper_orient = p.getLinkState(
                ...
                (
                    54
                    if self.robot_type == "pr2"
                    else 18 if self.robot_type == "sawyer" else 25 if self.robot_type == "baxter" else 8
                ),
                computeForwardKinematics=True,
                physicsClientId=self.id,
            )[:2]
        ...
        if self.task == "scratch_itch":
            tool = p.loadURDF(
                os.path.join(self.directory, "scratcher", "tool_scratch.urdf"),
            ...
            tool = p.loadURDF(
                os.path.join(self.directory, "bed_bathing", "wiper.urdf"),
                basePosition=transform_pos,
                baseOrientation=transform_orient,
                physicsClientId=self.id,
            )
            ...
            if self.task == "drinking":
                ...
                collision_filename = os.path.join(
                    self.directory, "arm_manipulation", "arm_manipulation_scooper_vhacd.obj"
                )
            ...
            tool_collision = p.createCollisionShape(
                shapeType=p.GEOM_MESH, fileName=collision_filename, meshScale=mesh_scale, physicsClientId=self.id
            )
        ...
        if left:
            ...
            for j in (
                ...
                if self.robot_type == "pr2"
                    ...
                    else [47, 49, 50, 51, 52] if self.robot_type == "baxter" else [7, 8, 9, 10, 11, 12, 13, 14]
                )
            ):
            ...
            for j in (
                ...
                if self.robot_type == "pr2"
                    ...
                    [18, 20, 21, 22, 23]
                    if self.robot_type == "sawyer"
                    else [25, 27, 28, 29, 30] if self.robot_type == "baxter" else [7, 8, 9, 10, 11, 12, 13, 14]
                )
            ):
                for tj in list(range(p.getNumJoints(tool, physicsClientId=self.id))) + [-1]:
                    p.setCollisionFilterPair(robot, tool, j, tj, False, physicsClientId=self.id)
    ...
    def setup_robot_joints(
        ...
        if randomize_joint_positions:
                ...
                for j, lower_limit, upper_limit in zip(robot_joint_indices, lower_limits, upper_limits):
                    ...
                    p.resetJointState(
                        ...
                        targetValue=self.np_random.uniform(
                            lower_limit + joint_range / 6.0, upper_limit - joint_range / 6.0
                        ),
                        targetVelocity=0,
                        physicsClientId=self.id,
                    )
            ...
            for i, j in enumerate(robot_joint_indices):
                p.resetJointState(
                    robot, jointIndex=j, targetValue=default_positions[i], targetVelocity=0, physicsClientId=self.id
                )
